function [StrainImage,SigmaImage,opts,edgeWidthImage,edgeWidthStdImage,Tr_cell] = makeStrainProfile(OB,Proj,opts,BraggOpts,d0Tr)
...
%           opts.sampleRange:   a 2 element vector containing the start and
%                               end pixel which define the extent of the sample.
%           opts.nWidth     :   Number of columns/row used for each edge.
...
%           opts.AveDir     :   Indicates whether the pixel averaging
%                               occurs in the vertical or horizontal
%                               direction ('horz','vert').
%       - BraggOpts is s structure generated by BraggOptions. It contains
%           the neccecary information for edge fitting.
%
%   Outputs:
...
%
% See also fitEdges, BraggOptions, ImageOptions.
%
...
for i = 1:(floor(nPixCol/opts.nRes))
    ...
    switch(lower(opts.AveDir))
        case 'horz'
            ...
            j_inds = opts.sampleRange(1):opts.sampleRange(2);
        case 'vert'
            ...
            i_inds = opts.sampleRange(1):opts.sampleRange(2);
    end
    %     i_inds = i;
    %     j_inds = opts.pixelCentre + (-opts.nPix:+1:opts.nPix);
